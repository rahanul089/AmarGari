"""
AmarGari - Database layer (SQLAlchemy models + session helper)

This app runs on Supabase (Postgres) only. Set DATABASE_URL either:
  - in Streamlit Cloud: App settings -> Secrets -> DATABASE_URL = "..."
  - locally: .streamlit/secrets.toml (copy from secrets.toml.example), or
    export DATABASE_URL=... in your shell

Use the Supabase "Connection pooling" URI from
Project Settings -> Database -> Connection string -> URI, e.g.:
  postgresql://postgres.xxxxxxxx:[PASSWORD]@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres
"""
import os
...
def _resolve_database_url() -> str:
    # 1. Streamlit secrets (works on Streamlit Community Cloud)
    try:
        ...
        pass
    # 2. Environment variable (local dev, or any other host)
    env_url = os.environ.get("DATABASE_URL")
    if env_url:
        return env_url
    raise RuntimeError(
        "DATABASE_URL is not set. This app requires a Supabase (Postgres) "
        "connection string. Set it in Streamlit Cloud under App settings -> "
        "Secrets, or locally in .streamlit/secrets.toml "
        "(see .streamlit/secrets.toml.example). See DEPLOYMENT.md for the "
        "full Supabase setup steps."
    )


DATABASE_URL = _resolve_database_url()

# Small pool since Streamlit apps can spin up multiple worker threads;
# pool_recycle avoids "server closed the connection unexpectedly" errors
# that show up with managed Postgres providers after idle periods.
...
def database_has_data() -> bool:
    """
    Used by app.py to decide whether to run the seed script.
    Works for both SQLite and an online Postgres DB (unlike checking
    for a local file, which only makes sense for SQLite).
    """
    db = get_session()
    try:
