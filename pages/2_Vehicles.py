...
from styles.components import vehicle_card

st.set_page_config(page_title="Vehicles - AmarGari", page_icon="🚙", layout="wide")
apply_theme()
...
with tab_list:
    ...
    if not vehicles:
        ...
        if vehicle_choice:
            ...
            selected = db.get(Vehicle, vid)

            card_col, form_col = st.columns([1, 1.4])
            with card_col:
                vehicle_card(
                    reg_number=selected.reg_number,
                    owner_name=selected.owner.name if selected.owner else user["name"],
                    vehicle_type=selected.vehicle_type or "—",
                    valid_till=str(selected.registered_on) if selected.registered_on else "—",
                )
            with form_col:
